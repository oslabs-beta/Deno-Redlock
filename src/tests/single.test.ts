  ...
  try {
    ...
    try {
      ...
      assertEquals(
        ...
        null,
        "new resource must remained unlocked.",
      );
      assertEquals(error.attempts.length, 11, "incorrect number of retries");
    ...
    assertEquals(
      await redis.get("{redlock}c1"),
      null,
      "c1 lock failed to release",
    );
    assertEquals(
      await redis.get("{redlock}c2"),
      null,
      "c2 lock failed to release",
    );
    assertEquals(
      await redis.get("{redlock}c3"),
      null,
      "c3 lock failed to release",
    );
  } catch (error) {
    fail(error);
  }

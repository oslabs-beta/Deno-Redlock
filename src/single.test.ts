...
Deno.test("acquires, extends, and releases a lock with a single resource", async () => {
  try {
    await redis
      .keys("*")
      .then((keys) => {
...
Deno.test("acquires, extends, and releases a multi-resource lock", async () => {
  try {
    await redis
      .keys("*")
      .then((keys) => {
    ...
    assertEquals(
      ...
      lock.value,
      "a1 lock value incorrect.",
    );
    assertEquals(
      await redis.get("{redlock}a2"),
      lock.value,
      "a2 lock value incorrect.",
    );
...
Deno.test("if communication with Redis goes down, the lock fails", async () => {
  try {
    await redis
      .keys("*")
      .then((keys) => {
...
Deno.test("automatic expiration for locks", async () => {
  try {
    await redis
      .keys("*")
      .then((keys) => {
...
Deno.test("locks are exclusive", async () => {
  try {
    await redis
      .keys("*")
      .then((keys) => {
...
Deno.test("overlapping multi-locks are exclusive", async () => {
  try {
    ...
    const lock = await redlock.acquire(
      ["{redlock}14", "{redlock}25"],
      duration,
    );
    assertEquals(
      await redis.get("{redlock}14"),
      lock.value,
      "lock not acquired successfully",
    );
    assertEquals(
      await redis.get("{redlock}25"),
      lock.value,
      "lock not acquired successfully",
    );
    ...
    try {
      ...
      assertEquals(
        await redis.get("{redlock}14"),
        lock.value,
        "original lock value shouldn't be changed.",
      );
      assertEquals(
        await redis.get("{redlock}25"),
        lock.value,
...
Deno.test("the using helper acquires, extends, and releases locks", async () => {
  try {
    await redis
      .keys("*")
      .then((keys) => {
    ...
    const duration = 500;
    let locked = false;
    const [valueP] = await Promise.all([
      await redlock.using(["{redlock}94"], duration, {
        ...
        assertEquals(
          signal.error,
          undefined,
          "signal shouldn't have an error.",
        );
        assertEquals(signal.aborted, false, "signal shouldn't be aborted.");
        assertEquals(
          await redis.get("{redlock}94"),
          lockValue,
          "lock value shouldn't have changed.",
        );
        return lockValue;
      }),
    ]);
...
Deno.test("the using helper is exclusive", async () => {
  try {
    await redis
      .keys("*")
      .then((keys) => {
    ...
    const [lock1, lock2] = await Promise.all([
      await redlock.using(["{redlock}y"], duration, {
        ...
        assertEquals(
          signal.error,
          undefined,
          "The signal shouldn't have an error.",
        );
      ...
      await redlock.using(["{redlock}y"], duration, {
        ...
        assertEquals(
          signal.error,
          undefined,
          "The signal must not have an error.",
        );
